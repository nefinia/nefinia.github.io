...
`;var g=document.querySelectorAll(`.lang-button`),_=document.querySelector(`#frequency-slider`),v=document.querySelector(`#frequency-readout`),y=document.querySelector(`#wavelength-readout`),b=document.querySelector(`#instrument-readout-card`),x=document.querySelector(`#instrument-readout`),S=document.querySelector(`#frequency-explanation`),ae=document.querySelector(`#spectrum-marker`),C=document.querySelector(`#mechanism-buttons`),w=document.querySelector(`#mechanism-process-image`),T=document.querySelector(`#mechanism-process-label`),E=document.querySelector(`#mechanism-process-license`),D=document.querySelector(`#mechanism-process-link`),O=document.querySelector(`#mechanism-source-image`),k=document.querySelector(`#mechanism-source-label`),A=document.querySelector(`#mechanism-source-license`),j=document.querySelector(`#mechanism-source-link`),M=document.querySelector(`#mechanism-tag`),N=document.querySelector(`#mechanism-name`),P=document.querySelector(`#mechanism-summary`),F=document.querySelector(`#mechanism-science`),I=document.querySelector(`#mechanism-where`),L=document.querySelector(`#instrument-tabs`),R=document.querySelector(`#instrument-image`),z=document.querySelector(`#instrument-band`),B=document.querySelector(`#instrument-name`),V=document.querySelector(`#instrument-tagline`),H=document.querySelector(`#instrument-facts`),U=document.querySelector(`#instrument-works`),oe=document.querySelector(`#instrument-finds`),W=document.querySelector(`#instrument-link`),G=document.querySelector(`#baseline-slider`),K=document.querySelector(`#lambda-slider`),q=document.querySelector(`#angle-slider`),J=document.querySelector(`#baseline-readout`),Y=document.querySelector(`#lambda-readout`),X=document.querySelector(`#angle-readout`),se=document.querySelector(`#delay-readout`),ce=document.querySelector(`#resolution-readout`),Z=document.querySelector(`#interferometry-canvas`);if(!_||!v||!y||!b||!x||!S||!ae||!C||!w||!T||!E||!D||!O||!k||!A||!j||!M||!N||!P||!F||!I||!L||!R||!z||!B||!V||!H||!U||!oe||!W||!G||!K||!q||!J||!Y||!X||!se||!ce||!Z)throw Error(`UI initialization failed.`);function le(){document.documentElement.lang=d,document.title=d===`en`?`Nançay Radio Observatory`:`Observatoire radioastronomique de Nançay`,document.querySelectorAll(`[data-i18n]`).forEach(e=>{e.textContent=s[e.dataset.i18n]?.[d]??``}),document.querySelectorAll(`[data-copy-en]`).forEach(e=>{e.textContent=e.dataset[d===`en`?`copyEn`:`copyFr`]??``}),g.forEach(e=>{e.classList.toggle(`is-active`,e.dataset.lang===d)})}function ue(){C.innerHTML=``,l.forEach(e=>{let t=document.createElement(`button`);t.type=`button`,t.className=`mechanism-button ${e.id===p?`is-active`:``}`,t.textContent=m(e.name),t.setAttribute(`role`,`tab`),t.setAttribute(`aria-selected`,`${e.id===p}`),t.addEventListener(`click`,()=>{p=e.id,Q(),ue()}),C.appendChild(t)})}function Q(){let e=l.find(e=>e.id===p)??l[0],t=u[e.id],n=ee[e.id]??e.visual;M.textContent=d===`en`?`Emission mechanism`:`Mécanisme d’émission`,N.textContent=m(e.name),P.textContent=m(e.summary),F.textContent=m(e.science),I.textContent=m(e.where),w.src=t.src,w.alt=t.label,T.textContent=t.label,E.textContent=t.license,D.href=t.source,O.src=n.src,O.alt=n.label,k.textContent=n.label,A.textContent=n.license,j.href=n.source}function de(){L.innerHTML=``,c.forEach(e=>{let t=document.createElement(`button`);t.type=`button`,t.className=`instrument-tab ${e.id===f?`is-active`:``}`,t.innerHTML=`<strong>${e.name}</strong><span>${e.wavelengths}</span>`,t.addEventListener(`click`,()=>{f=e.id,fe(),de()}),L.appendChild(t)})}function fe(){let e=c.find(e=>e.id===f)??c[0];R.src=e.image,R.alt=e.alt,z.textContent=`${e.rangeMHz[0]}-${e.rangeMHz[1]} MHz · ${e.wavelengths}`,B.textContent=e.name,V.textContent=m(e.tagline),H.innerHTML=e.facts.map(e=>`<p>${m(e)}</p>`).join(``),U.textContent=m(e.works),oe.innerHTML=e.finds.map(e=>`<li>${m(e)}</li>`).join(``),W.href=e.link}function pe(){let e=Number(_.value),t=ne(e),n=ie(e),r=(Math.log10(e)-Math.log10(10))/(Math.log10(3500)-Math.log10(10))*36;if(v.textContent=e>=1e3?`${(e/1e3).toFixed(2)} GHz`:`${e.toFixed(0)} MHz`,y.textContent=re(t),ae.style.left=`${Math.max(0,Math.min(100,r))}%`,n.length){let e=[...new Set(n.flatMap(e=>e.physics))];b.hidden=!1,x.textContent=n.map(e=>e.name).join(`, `),S.textContent=d===`en`?`At this frequency, these Nançay instruments can observe: ${n.map(e=>e.name).join(`, `)}. The science shifts toward: ${e.join(`, `)}.`:`À cette fréquence, ces instruments de Nançay peuvent observer : ${n.map(e=>e.name).join(`, `)}. La science se déplace vers : ${e.join(`, `)}.`,S.hidden=!1}else b.hidden=!0,x.textContent=``,S.textContent=``,S.hidden=!0}function me(e){return e*206264.806}function $(){let e=Number(G.value),t=Number(K.value),n=Number(q.value),r=n*Math.PI/180,i=e*1e3,a=i*Math.sin(r)/299792458,o=me(t/i);J.textContent=`${e.toFixed(0)} km`,Y.textContent=re(t),X.textContent=`${n.toFixed(0)}°`,se.textContent=`${(a*1e6).toFixed(2)} µs`,ce.textContent=o>=1?`${o.toFixed(2)} arcsec`:`${(o*1e3).toFixed(1)} mas`,he(r)}function he(e){let t=Z.getContext(`2d`);if(!t)return;let n=window.devicePixelRatio||1,r=Math.max(320,Math.floor(Z.getBoundingClientRect().width)),i=Math.round(r*.62);Z.width=r*n,Z.height=i*n,Z.style.height=`${i}px`,t.setTransform(n,0,0,n,0,0),t.clearRect(0,0,r,i),t.fillStyle=`#f7f3ea`,t.fillRect(0,0,r,i);let a=i*.76,o=r*.32,s=r*.68,c=-Math.PI/2+e,l=Math.cos(c),u=Math.sin(c);t.strokeStyle=`#b8cad0`,t.lineWidth=2;for(let n=-4;n<=8;n+=1){let a=r*.5+n*54+Math.sin(e)*35,o=i*.08;t.beginPath(),t.moveTo(a-u*360,o+l*360),t.lineTo(a+u*360,o-l*360),t.stroke()}t.strokeStyle=`#263b42`,t.lineWidth=4,t.beginPath(),t.moveTo(40,a),t.lineTo(r-40,a),t.stroke(),[o,s].forEach(e=>{t.strokeStyle=`#24526a`,t.lineWidth=8,t.beginPath(),t.moveTo(e,a),t.lineTo(e,a-80),t.moveTo(e,a-78),t.lineTo(e-48,a-128),t.moveTo(e,a-78),t.lineTo(e+48,a-128),t.stroke(),t.fillStyle=`#2f6f73`,t.beginPath(),t.arc(e,a-82,12,0,Math.PI*2),t.fill()}),t.strokeStyle=`#be6a2f`,t.lineWidth=5,t.beginPath(),t.moveTo(r*.16,i*.16),t.lineTo(r*.16+l*135,i*.16+u*135),t.stroke(),t.fillStyle=`#be6a2f`,t.beginPath(),t.arc(r*.16+l*135,i*.16+u*135,8,0,Math.PI*2),t.fill(),t.fillStyle=`#123143`,t.font=`600 14px Inter, system-ui, sans-serif`,t.fillText(d===`en`?`incoming wavefronts`:`fronts d’onde entrants`,24,34),t.fillText(d===`en`?`baseline B`:`ligne de base B`,r*.45,a+30)}function ge(){le(),ue(),Q(),de(),fe(),pe(),$()}g.forEach(e=>{e.addEventListener(`click`,()=>{d=e.dataset.lang===`fr`?`fr`:`en`,ge()})}),_.addEventListener(`input`,pe),G.addEventListener(`input`,$),K.addEventListener(`input`,$),q.addEventListener(`input`,$),window.addEventListener(`resize`,()=>{$()}),ge();